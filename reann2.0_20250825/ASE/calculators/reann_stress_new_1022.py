...
class REANN(Calculator):
    ...
    def __init__(self,period=[1,1,1],device='cpu',nn = 'REANN_PES_FLOAT.pt',**kwargs):
    ...
    def transfer2eann(self,atoms):
        ...
        for sp in spe1:
            ...
            spe.append(s[0])
            #print(s,num_atom)
        #print("number_atom",number_atom)


        return cell,cart,number_atom,spe
    
    def eann_all_str(self,number_atom,spe):
        species = []
        ...
        for i in range(len(spe)):
            for i1 in range(number_atom[i]):
                species.append(Atom(spe[i]).number)
                mass.append(Atom(spe[i]).mass)
        #print("mass",mass)
...
#                    species.append(Atom(tmp[0]yuansufuhao).number)
...
#                    break
    ...
    def calculate(self,atoms=None, properties=['energy'],
        ...
        species,mass = self.eann_all_str(number_atom,spe)
        period_table=torch.tensor(period,dtype=torch.float32,device=device)
