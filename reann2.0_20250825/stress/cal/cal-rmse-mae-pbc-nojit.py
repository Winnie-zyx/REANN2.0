...
path_env=path_env.replace("stress/cal","")
print("path_env ",path_env)
sys.path.append(path_env)

import stress.PES as PES
...
if datatype=="double":
    ...
    pes=init_pes #pes=torch.jit.load(path_model+"REANN_PES_DOUBLE.pt")
else:
    tensor_type = torch.float
    pes=init_pes #pes=torch.jit.load(path_model+"REANN_PES_FLOAT.pt")
...
with open(path_dat,'r') as f1:
    while True:
        ...
        while True:
            ...
            species.append(Atom(ele_tmp).number) #species.append(atomtype.index(tmp[0]))
            numatom+=1
        #
