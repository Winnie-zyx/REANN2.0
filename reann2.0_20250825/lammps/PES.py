...
class PES(torch.nn.Module):
    ...
    def forward(self,cart,cell,atom_index,local_species,neigh_species):
        atom_index = atom_index.t().contiguous()
        cart.requires_grad_(True)
        #density,dist_vec=self.density(cart,atom_index,local_species,neigh_species)
        #output = self.nnmod(density,local_species)+self.nnmod.initpot

        dist_vec,output=self.getdensity(cart,atom_index,local_species,neigh_species)
