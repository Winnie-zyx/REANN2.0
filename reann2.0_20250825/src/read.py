...
if start_table<=2 or start_table==5:
   outputneuron=1
elif start_table==3:
   outputneuron=3
elif start_table==4:
...
if start_table==0 or start_table==1:
    ...
    stress=pot_stress[:,1:]/(-1)#/1602.1766208  #.reshape(-1,3,3)
else: # 4
...
numpoint=np.array(numpoint,dtype=np.int32)
numatoms=np.array(numatoms,dtype=np.int32)
...
if start_table==1: 
    ...
    volume=torch.sum(scalmatrix_t[:, 0, :] * torch.cross(scalmatrix_t[:, 1, :], scalmatrix_t[:, 2, :], dim=1),dim=1)
    volume_train=volume[range_train[0]:range_train[1]]
...
if start_table==5: # static stress tensor
    ...
    abpropset_train=(pot_train,stress_train,force_train)
    abpropset_test=(pot_test,stress_test,force_test)
    nprop=3
    test_nele=torch.empty(nprop)
    train_nele=torch.empty(nprop)
    train_nele[0]=numpoint[0]
    train_nele[1]=numpoint[0]*9
    train_nele[2]=ntrain_vec
    test_nele[0]=numpoint[1] 
    test_nele[1]=numpoint[1]*9
    test_nele[2]=ntest_vec
    force_train_size=force_train.size(1)
    force_test_size=force_test.size(1)
...
if start_table==5: del pot_stress,force
gc.collect()
...
prop_ceff[1]=init_f
if start_table==5:
    prop_ceff=torch.ones(3,device=device)
    prop_ceff[0]=e_ceff
    prop_ceff[1]=init_f*10
    prop_ceff[2]=init_f
