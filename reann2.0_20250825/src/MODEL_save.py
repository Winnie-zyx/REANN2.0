   ...
   def __init__(self,maxnumtype,outputneuron,atomtype,nblock,nl,dropout_p,actfun,initpot=0.0,table_norm=True):
      ...
      with torch.no_grad():
          ...
          for ele in atomtype:
              ...
              modules.append(linear)
              elemental_nets[ele] = Sequential(*modules)    ##按顺序保存module，之后索引的是module，是一个列表封装成的字典
...
#   @pysnooper.snoop('out',depth=2)   for debug
   def forward(self,density,species):    
      ...
      output = torch.zeros((density.shape[0],self.outputneuron), dtype=density.dtype, device=density.device)   ##给布尔值为0的矩阵
      for itype, (_, m) in enumerate(self.elemental_nets.items()):        ##itype是列表的索引，打开以后(_,m)是键值对，只关注值，所以键用_，enumerate是把字典变成列表
          mask = (species == itype)    ##species=itype的位置mask=1
          ele_index = torch.nonzero(mask).view(-1)     ##mask不为0的ele_index有值
          if ele_index.shape[0] > 0:
              ele_den = density[ele_index]      ##对应元素索引的density
              output[ele_index] = m(ele_den)       ##输出的是原子能量值，m对应的值，每总原子内部加和到一起了
      return output
