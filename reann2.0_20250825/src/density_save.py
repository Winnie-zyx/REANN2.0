...
class GetDensity(torch.nn.Module):
    ...
    def forward(self,cart,numatoms,species,atom_index,shifts):
        ...
        id_config=torch.arange(atom_index.shape[1],device=cart.device).reshape(-1,1).repeat(1,atom_index.shape[2]).reshape(-1)
        ...
        #print(species_)
        #print(species_.size())
        nangular=self.angular(dist_vec,self.cutoff_cosine(distances))
        orbital = oe.contract("ji,ik -> ijk",self.angular(dist_vec,self.cutoff_cosine(distances)),\
        self.gaussian(distances,species_),backend="torch")    ##orbital:[neigh,lenth,nwave]
        #print(atom_index12.size())
        #print((self.angular(dist_vec,self.cutoff_cosine(distances))).size())
        ...
        return density,dist_vec,selected_id_config
