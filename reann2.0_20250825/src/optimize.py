...
data_train,data_test,force_train_size,force_test_size,Prop_class,loss_fn,optim,scheduler,ema,restart,PES_Normal,device,PES_Lammps=None): 
    ...
    force_train_sizee = torch.tensor([force_train_size]).to(device)
    force_test_sizee = torch.tensor([force_test_size]).to(device)
    ...
    for iepoch in range(Epoch): 
       ...
       for data in data_train:
          abProp,cart,numatoms,species,atom_index,shifts,volume=data
          loss=loss_fn(Prop_class(cart,numatoms,species,atom_index,shifts,volume,force_train_sizee),abProp)
          #loss=loss_fn(Prop_class(cart,numatoms,species,atom_index,shifts,volume,\
          #     create_graph=False),abProp)
          lossprop+=loss.detach().data
       ...
       if np.mod(iepoch,print_epoch)==0:
          ...
          for data in data_test:
             abProp,cart,numatoms,species,atom_index,shifts,volume=data
             loss=loss_fn(Prop_class(cart,numatoms,species,atom_index,shifts,volume,force_test_sizee,\
             create_graph=False),abProp)
             lossprop=lossprop.data+loss.detach().data
          ...
          if nprop==3:
              prop_ceff[1]=f_ceff*10
              prop_ceff[2]=f_ceff
          if nprop==2:
              prop_ceff[1]=f_ceff
          #  save the best model
          if loss<best_loss[0]:
             best_loss[0]=loss.detach().data
             if rank == 0:
          ...
          if rank==0:
              lossprop=torch.sqrt(lossprop.cpu()/test_nele)
