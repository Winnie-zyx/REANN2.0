...
class GetDensity(torch.nn.Module):
    def __init__(self,rs,inta,neigh_atoms,initpot=0,nipsin=3,nwave=8,cutoff=4.0,norbit,emb_nblock=1,emb_nl=[1,8,8],emb_layernorm=True,oc_loop=3,oc_nblock=1,oc_nl=[64,64,64],oc_dropout_p=[0.0,0.0],oc_layernorm=True,nblock=1,nl=[64,64,64],dropout_p=[0.0,0.0],layernorm=True,Dtype=torch.float32):   ##add atom_species, delete rs and inta, atom_species from inputand transport to torch,tensor([[8.],[1.]])
        ...
        #embedded nn
        self.emb_neighnn=NNMod(self.nwave*3,emb_nblock,emb_nl,np.array([0]),Relu_like,initbias=initbias,layernorm=emb_layernorm)          ##emb_nl should be given in input
    ...
    def forward(self,cart,numatoms,species,atom_index,shifts):
        ...
        nangular=self.angular(dist_vec,cut_distances)   ##[neigh,lenth]  lenth=npara[0]+npara[1]+...+npara[ipsin]
        orbital = oe.contract("ij,ki -> ijk",nangular,radial_func,backend="torch")    ##orbital:[neigh,lenth,nwave]
...
#----------------output calculation------------------------------------
        ...
        index = torch.nonzero(mask_species).view(-1)
        output_= self.outnn(density)    ##[numatoms,norbit]
        out = torch.einsum("ij,i->ij",output_,index)
