...
if start_table==0:
    ...
    from src.Property_stress import *
...
species_train,atom_index_train,shifts_train,volume_train,batchsize_train,min_data_len=min_data_len_train,shuffle=True)
#=================================test data loader=================================
dataloader_test=DataLoader(com_coor_test,abpropset_test,numatoms_test,\
species_test,atom_index_test,shifts_test,volume_train,batchsize_test,min_data_len=min_data_len_test,shuffle=False)
# dataloader used for load the mini-batch data
#print("point1")
#print(species_test.size())
...
#=======================density======================================================
getdensity=GetDensity(rs,inta,neigh_atoms,initpot=0,nipsin,nwave=8,cutoff,norbit,emb_nblock=1,emb_nl=[1,8,8],emb_layernorm=True,oc_loop=3,oc_nblock=1,oc_nl=[64,64,64],oc_dropout_p=[0.0,0.0],oc_layernorm=True,nblock=1,nl=[64,64,64],dropout_p=[0.0,0.0],layernorm=True,Dtype=torch.float32)
...
#=========================create the module=========================================
Prop_class=Property(getdensity).to(device)  # to device must be included
