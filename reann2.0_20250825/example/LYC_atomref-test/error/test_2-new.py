...
import sys
sys.path.append("/public/home/zyx/.local/lib/python3.10/site-packages/")
...
strss=np.zeros((3,3),dtype=np.float32)
period_table=torch.tensor([1,1,1],dtype=torch.float32,device=device)   # same as the pbc in the periodic boundary condition

rmse=torch.zeros(3,dtype=torch.float32,device=device)
mae=torch.zeros(3,dtype=torch.float32,device=device)
#i=0
#sig=0
...
#with open("/public/home/zyx/dataset/mp_trj_extxyz_by_yuan/reann/1/TEST/configuration",'r') as f1:
with open("../data/test/configuration",'r') as f1:
...
##                abstress.append(tmp1[:])
                ...
                species.append(Atom(tmp[0]).number)
            ...
            mass=torch.from_numpy(np.array(mass)).to(device).to(torch.float32)  # also float32/double
            abforce=torch.from_numpy(np.array(abforce)).to(device).to(torch.float32)  # also float32/double
            abstress=torch.from_numpy(np.array(abstress)).to(device).to(torch.float32)
#            print(abstress)
...
#                  tt.write("%18.8f\n" %(i))
                ...
                mae[0]+=torch.sum(torch.abs(energy-abene))
