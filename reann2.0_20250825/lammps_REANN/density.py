...
class GetDensity(torch.nn.Module):
    ...
    def forward(self,cart,atom_index,local_species,neigh_list):
        ...
        #
        #species_1=local_species.add(1)
        num_classes = 118
        #one_hot_encodings = (torch.nn.functional.one_hot(species_1, num_classes)).float() #nlocal,118
        one_hot_encodings = (torch.nn.functional.one_hot(local_species.add(1), num_classes)).float() #nlocal,118
        #
        center_coeff=self.emb_centernn(one_hot_encodings)
        expand_spec_0=torch.index_select(one_hot_encodings,0,(atom_index[0])).view(-1,num_classes)  #problem
        #expand_spec_1=torch.index_select(one_hot_encodings,0,neigh_list.view(-1)).view(-1,num_classes)
        expand_spec_1=torch.index_select(one_hot_encodings,0,neigh_list).view(-1,num_classes)
