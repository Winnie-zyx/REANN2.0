...
class GetDensity(torch.nn.Module):
    ...
    def forward(self,cart,neigh_list,shifts,species):     ##neigh_list=atom_index
        ...
        for ioc_loop, (_, m) in enumerate(self.ocmod.items()):
            ...
            iter_coeff = iter_coeff + torch.index_select(nnout,0,neigh_list[1])
            #print(iter_coeff)
            #print(m(density))
            density,center_orbital=self.density0(orbital,cut_distances,iter_coeff,neigh_list[0],neigh_list[1],contracted_coeff[ioc_loop+1],zero_orbital,center_orbital,center_coeff)
